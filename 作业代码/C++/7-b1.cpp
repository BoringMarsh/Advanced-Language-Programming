...
/* 可以在此定义其它需要的函数 */
/***************************************************************************
  函数名称：leap_year_check
  功    能：输入年份，判断是否为闰年
  输入参数：const int year：待判断的年份（只读）
  返 回 值：bool：是否为闰年
  说    明：无
***************************************************************************/
...
struct tj_time *tj_time_convert(int input_time)
{
	...
	/*2、满60分钟进一小时，余数是最终结果的分*/
	while (result.tj_minute >= 60) {
		result.tj_hour++;
		result.tj_minute = result.tj_minute - 60;
	}

	/*3、满24小时进一天，余数是最终结果的时*/
	while (result.tj_hour >= 24) {
		result.tj_day++;
		result.tj_hour = result.tj_hour - 24;
	}

	/*4、超过365/366天进一年，得到最终结果的年*/
	while (1) {
		bool prime = leap_year_check(result.tj_year);  /*判断闰年*/

		if (result.tj_day > 365 + prime) {  /*满365/366天进一年*/
			result.tj_day = result.tj_day - (365 + prime);
			result.tj_year++;
		}

		if (result.tj_day <= 365 + prime)  /*天数不足，结束循环*/
			break;
	}
	...
	/*6、超过一个月的天数进一月，得到最终结果的月，余数为最终结果的日*/
